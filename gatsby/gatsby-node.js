...
exports.onCreateWebpackConfig = ({ stage, loaders, actions, getConfig }, pluginOptions) => {
  if (stage === 'develop-html') return

  const config = getConfig()

  const outputCSS = !stage.includes('html')
  const inlineLoader = stringifyCssRequest([loaders.miniCssExtract(), { loader: 'css-loader' }])
