...
function flattenClasses(classes) {
  return Object.fromEntries(
    Object.entries(classes).map(([key, value]) => {
      const objValues = Object.fromEntries(flattenStyles(value).map(({ value, ...rest }) => [JSON.stringify(rest), value]))
...
function replaceUseCalls(varDec, classes) {
  ...
  for (const use of uses) {
    if (use.parentPath.isCallExpression() && use.parent.callee === use.node) {
      ...
      getDynamicOrStaticKeys(use.parentPath, Object.keys(classes)).forEach((key) => names.add(key))
      ...
      if (!isHMR) {
        throw use.buildCodeFrameError('Return value from style9.create has to be called as a function or ' + 'accessed as an object')
      }
    }
  }
...
function astFromObject(obj) {
  ...
  for (const name in obj) {
    const astValue = typeof obj[name] === 'object' ? astFromObject(obj[name]) : t.stringLiteral(obj[name])
    const key = t.isValidIdentifier(name, false) ? t.identifier(name) : t.stringLiteral(name)
...
function generateStyles(styles) {
  return Object.values(styles).flatMap((props) => flattenStyles(props).map(getDeclaration))
}
...
function minifyProperties(classes) {
  ...
  for (const key in classes) {
    ...
    minified[minifiedName] = typeof value === 'object' ? minifyProperties(value) : value
  }
...
function handleCreate(identifier, opts) {
  ...
  if (opts.minifyProperties) {
    const minifiedClasses = Object.fromEntries(Object.entries(classes).map(([key, value]) => [key, minifyProperties(value)]))
